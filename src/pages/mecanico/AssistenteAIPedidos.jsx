...
import { produtoAPI, pedidoAPI, handleApiError } from '../../services/api';
...
const ChatbotPedidos = () => {
  ...
  const processarInputUsuario = async () => {
    ...
    try {
      ...
      const todosProdutos = await produtoAPI.findAll();
      const resultados = todosProdutos.filter(p => 
        p.nome.toLowerCase().includes(inputLower) || 
        p.categoria?.toLowerCase().includes(inputLower) ||
        p.id?.toString().includes(inputLower)
      );
      ...
      setMensagens(prev => prev.filter(m => !(m.autor === 'bot' && m.tipo === 'componente')));
      adicionarMensagemBot(`Erro ao buscar produtos: ${errorInfo.message}`);
      setEstadoConversa('inicial');
    } finally {
  ...
  const handleFinalizarCompra = async (e) => {
    ...
    try {
      ...
      setMensagens(prev => [
        ...
        { 
          texto: `Pedido #${pedidoCriado.id} realizado com sucesso! Obrigado por comprar na Peça Já!`, 
          autor: 'bot', 
          id: Date.now(), 
          tipo: 'texto' 
        }
  ...
  return (
    <div className="relative flex flex-col h-[calc(100vh-8rem)] bg-white rounded-lg shadow-xl overflow-hidden border border-gray-200">
      <div className="flex justify-between items-center pt-6 pb-4 px-4 sm:px-6 flex-shrink-0">
        <h1 className="text-2xl font-bold text-gray-800">Faça seu pedido!</h1>
        <button
                ...
                <div key={item.produto.id} className="flex items-start border-b border-gray-200 pb-4 last:border-b-0">
                  ...
                  <div className="flex-1 flex flex-col">
                    <div className="flex justify-between text-base font-medium text-gray-900">
                      ...
                      <p className="ml-4 whitespace-nowrap font-semibold">R$ {(item.produto.preco * item.quantidade).toFixed(2)}</p>
                    </div>
                    ...
                    <div className="flex flex-1 items-end justify-between text-sm mt-3">
                      <div className="flex items-center space-x-2 border border-gray-300 rounded">
                        <button onClick={() => alterarQuantidade(item.produto.id, -1)} className="px-2 py-0.5 text-gray-600 hover:text-red-600">-</button>
                        <span className="text-gray-900 font-medium w-6 text-center">{item.quantidade}</span>
                        <button onClick={() => alterarQuantidade(item.produto.id, 1)} className="px-2 py-0.5 text-gray-600 hover:text-green-600">+</button>
                      </div>
                      <button type="button" onClick={() => removerDoCarrinho(item.produto.id)} className="font-medium text-xs text-red-600 hover:text-red-500">Remover</button>
                    </div>
                  </div>
                </div>
            ...
            {carrinho.length > 0 && (
              <div className="border-t border-gray-200 px-6 py-4 bg-gray-50">
                <div className="flex justify-between text-lg font-semibold text-gray-900 mb-4">
                  <p>Total:</p>
                  <p>R$ {calcularTotalCarrinho().toFixed(2)}</p>
                </div>
        ...
        <div className="fixed inset-0 bg-black/60 backdrop-blur-sm flex items-center justify-center z-[60] p-4">
          <div className="bg-white rounded-xl shadow-xl max-w-md w-full max-h-[90vh] flex flex-col overflow-hidden">
            ...
            <form onSubmit={handleFinalizarCompra} className="flex-1 overflow-y-auto p-6 space-y-6">
              <div>
                <h4 className="text-base font-medium text-gray-700 mb-3">Resumo</h4>
                {carrinho.map(item => (
                  <div key={item.produto.id} className="flex justify-between items-center text-sm mb-1 text-gray-600">
                    ...
                    <span className="whitespace-nowrap">R$ {(item.produto.preco * item.quantidade).toFixed(2)}</span>
                  </div>
                ))}
              ...
              <hr />
              <div>
                <label htmlFor="checkoutNome" className="block text-sm font-medium text-gray-700 mb-1">Nome <span className="text-red-500">*</span></label>
              ...
              <div className="pt-4 border-t border-gray-200">
                <button type="submit" className="w-full flex items-center justify-center rounded-lg border border-transparent bg-red-600 px-6 py-3 text-base font-medium text-white shadow-sm hover:bg-red-700 transition-colors disabled:opacity-70 disabled:cursor-not-allowed" disabled={processandoCheckout || carrinho.length === 0}>
                  {processandoCheckout ? (<><Loader2 className="animate-spin -ml-1 mr-3 h-5 w-5 text-white" />Processando...</>) : ('Confirmar Compra')}
                </button>
              </div>
            </form>
          </div>
